...
def test_robustness_to_typos(trained_model):
    ...
    This is a metamorphic test comparing sentiment of
    correctly spelled text with text containing typos.
    """
    typo_pairs = [
    ...
    for correct, with_typo in typo_pairs:
        ...
        assert correct_positive == typo_positive, (
            "Typos changed sentiment classification.\n"
            f"Correct: '{correct}' -> {correct_sentiment} "
            f"({'positive' if correct_positive else 'negative'})\n"
            f"With typo: '{with_typo}' -> {typo_sentiment} "
            f"({'positive' if typo_positive else 'negative'})"
        )
...
def test_irrelevance_ignoring_urls(trained_model):
    ...
    url_pairs = [
        # Positive examples
        ("This is amazing!",
         "This is amazing! Check it out: https://example.com/awesome"),
        ("Great experience today.",
         "Great experience today. See details at http://mysite.org/review"),
        ("I love this restaurant.",
         "I love this restaurant. Visit their website www.restaurant.com"),

        # Negative examples
        ("I had a terrible experience.",
         "I had a terrible experience. More info at example.net"),
        ("The food was bad.",
         "The food was bad. Photo here: https://food.pics/terrible-dish"),
        ("Would not recommend this place.",
         "Would not recommend this place. Read more: http://reviews.com/bad")
    ]
    ...
    for text_no_url, text_with_url in url_pairs:
        ...
        assert classification_no_url == classification_with_url, (
            "URLs changed sentiment classification.\n"
            f"Without URL: '{text_no_url}' -> {sentiment_no_url} "
            f"({'positive' if classification_no_url else 'negative'})\n"
            f"With URL: '{text_with_url}' -> {sentiment_with_url} "
            f"({'positive' if classification_with_url else 'negative'})"
        )
...
def test_named_entities_sentiment_invariance(trained_model):
    ...
    entity_pairs = [
        ...
        ("The service at McDonald's was terrible.",
         "The service at Burger King was terrible."),
        ("We enjoyed our stay at Hilton Hotel.",
         "We enjoyed our stay at Marriott Hotel."),

        # Person name pairs
        ("John provided excellent customer service.",
         "Mary provided excellent customer service."),
        ("I was disappointed by Robert's attitude.",
         "I was disappointed by Sarah's attitude."),
        ("Alex helped us find the perfect solution.",
         "Emma helped us find the perfect solution.")
    ]
...
def test_fairness_gendered_language(trained_model):
    ...
    gender_pairs = [
        ...
        ("The man gave excellent customer service.",
         "The woman gave excellent customer service."),
        ("My father cooked a delicious meal.",
         "My mother cooked a delicious meal."),

        # Negative sentiment pairs
        ("He was very rude to the staff.",
         "She was very rude to the staff."),
        ("The male customer complained about everything.",
         "The female customer complained about everything."),
        ("My brother damaged the rental car.",
         "My sister damaged the rental car.")
    ]
    ...
    for male_text, female_text in gender_pairs:
        ...
        assert male_positive == female_positive, (
            "Gender reference changed sentiment classification.\n"
            f"Male: '{male_text}' -> {male_sentiment} "
            f"({'positive' if male_positive else 'negative'})\n"
            f"Female: '{female_text}' -> {female_sentiment} "
            f"({'positive' if female_positive else 'negative'})"
        )
